...
int main(int argv, char *args[]) {
  ...
  tTimer->ResetStart();
	FileWriter *resultWriter=new FileWriter(args[14],false);
	cout<<"tSetSize="<<tSetSize<<endl;
  for (j = 0; j < tSetSize; j++) {
    // cout<<"Query "<<j<<":"<<endl;
    tQuery->Window(querySetlat1[j], querySetlng1[j], querySetlat2[j],
                   querySetlng2[j],resultWriter);
